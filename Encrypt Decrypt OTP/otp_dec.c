...
int main(int argc, char *argv[])
{
	...
	if(returncode != 'Z'){
		...
		decryptiontext = malloc(filelength + 1);

		memset(decryptiontext, '\0', filelength + 1);
		...
		while(total < filelength){
			...
			recievelength = recv(socketFD, decryptiontext + total, filelength, 0); // sending text string
		...
		decryptiontext[filelength-1] = '\n';
    	//decryptiontext[filelength+1] = '\0';

    	//fprintf(stdout, "The decryptiontext for client is: %s\n", decryptiontext);
		//fprintf(stdout, "The stringlength of decryptiontext for client is: %d\n", strlen(decryptiontext));

		if(outputfd != NULL){
			fprintf(outputfd, decryptiontext, filelength);	

		}

		else{
			fprintf(stdout, decryptiontext, filelength);
		}
		...
		free(textstring);
		free(decryptiontext);
		//fprintf(stdout,"The recieving text is: %s\n", encryptiontext);
